...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', 'ejrp-dev-key-change-in-prod')

if _BOOT_ERROR:
    @app.route('/')
    @app.route('/<path:p>')
    def boot_error(p=''):
        return f'<pre style="background:#0d1117;color:#ef4444;padding:20px;font-family:monospace">EJRP Boot Error:\n\n{_BOOT_ERROR}</pre>', 500

@app.before_request
def check_api_key():
    api_key = os.environ.get('EJRP_API_KEY')
...
def health_check():
    result = {'status': 'ok', 'app': 'EJRP', 'python': sys.version}
    try:
        conn = get_db()
        cur = conn.cursor()
